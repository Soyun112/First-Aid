...
def build_prompt(req: GenerateMessageRequest) -> str:
    ...
    route_line = (
        f"- 출발지: {origin}\n- 도착지: {destination}"
        if origin
        else f"- 도착지: {destination}"
    )

    return f"""당신은 병원에서 이동식 침대로 이송되는 환자를 안심시키는 따뜻한 안내 멘트를 작성합니다.

다음 환자 정보를 반영해, 이송 중 음성으로 읽어줄 **한국어 구어체 존댓말** 멘트를 작성하세요.
**총 2~3문장**으로 작성하고 **3문장을 넘기지 마세요**.
...
[문장 구성]
1. 첫 문장: 지금 어디로 이동 중인지 안내 (도착지「{destination}」를 자연스럽게 포함)
